...
import useEditShopForm from '@/hooks/useEditShopForm';
...
const Create = ({ shop, statuses, operationals }: Props) => {
    const title = 'Ubah Data Toko';

    const [preview, setPreview] = useState<string | null>(
        shop.image ? `/storage/shops/${shop.image}` : null,
    );
    const { data, setData, post, processing, errors } = useEditShopForm(shop);

    const handleSubmit: FormEventHandler = (e) => {
        e.preventDefault();
        post(shops.update.url(shop.encrypted_id), {
            preserveScroll: true,
            forceFormData: true,
            onSuccess: () => {
